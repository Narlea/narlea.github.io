...
async function showDetail(title) {
    ...
    if (detailContainer) {
        ...
        // 【关键】文章跳转也需要触发 active 类，否则会因为 opacity: 0 看不见
    ...
    // 动态添加打字机光标类
    if (titleEl) titleEl.classList.add('typing'); 
...
window.showHome = function() {
    switchLayoutMode('home');
    const postList = document.getElementById("post-list");
    const detailContainer = document.getElementById("post-detail");
    const detailBody = document.getElementById("detail-body");

    // 1. 关键修复：恢复显示的同时，必须把透明度重置为 1
    if (postList) {
    ...
    // 2. 隐藏详情容器，并移除其 active 类（方便下次进入时再次触发动画）
    if (detailContainer) {
...
async function navigateToModule(url) {
    ...
    // 1. 启动切换动画：让当前内容先变透明
    if (postList) postList.style.opacity = "0";
    detailContainer.classList.remove('active');

    // 2. 延迟执行切换逻辑，等待淡出效果完成（300ms）
    setTimeout(async () => {
        // 切换显示状态
        if (postList) postList.style.display = "none";
        ...
        // 处理标题和光标特效
        if (titleEl) {
            titleEl.classList.remove('typing'); // 移除光标
            titleEl.innerHTML = ""; // 清空工具页标题
        }

        detailBody.innerHTML = '<div class="system-msg">>> INITIALIZING_MODULE...</div>';
        ...
        try {
            ...
            // --- 激活新页面中的脚本 (保持你原有的逻辑) ---
            const scripts = detailBody.querySelectorAll('script');
            scripts.forEach(oldScript => {
                const newScript = document.createElement("script");
                if (oldScript.src) {
            ...
            // 激活内联样式
            const styles = detailBody.querySelectorAll('style');
            styles.forEach(s => {
            ...
            detailBody.innerHTML = `<div class="system-msg" style="color:var(--neon-blue)">[ ERROR ]:<br>无法加载模块: ${url}</div>`;
        }
    }, 300);
}
/**
 * 布局状态切换器
 * @param {string} mode - 'home' 或 'tool'
 */
function switchLayoutMode(mode) {
    if (mode === 'home') {
        document.body.classList.remove('focus-mode');
        // 恢复背景尺寸，增加从专注到发散的视觉效果
